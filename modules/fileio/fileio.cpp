...
string FileIO::prep_line(ParsedNode& Node) {
    stringstream stream ;
    if (Node.get_address() >= 0xFFFF ) {
        return "";
    }
    stream << ":04"
    << std::setfill('0') << std::setw(sizeof(int32_t) )
    << std::uppercase << std::hex << Node.get_address();
    
    stream << "00" << std::setfill('0') << std::setw(sizeof(int16_t))
    << std::uppercase << std::hex << Node.get_instruction();
    ...
    sum &= 0xFF;
    sum = ~sum + 1;
    stream << setfill('0') << setw(sizeof(short int)) << std::uppercase << std::hex <<(short int) sum << "\n";
    return stream.str();
}
